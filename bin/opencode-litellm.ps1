...
function Ensure-BaseUrl {
    ...
    if (-not (Test-BaseUrl)) {
        ...
        Write-LOk "已套用 LITELLM_BASE_URL ($value)"
    }
}
...
function Run-Config {
    Write-LInfo '重新設定 LiteLLM API Key'
    Write-Host ''
    Ensure-EnvFile
    Ensure-BaseUrl
    Ensure-ProviderName
...
function Run-FirstTimeSetupIfNeeded {
    # 自動套用預設 URL 與 Provider Name
    Ensure-EnvFile
    Ensure-BaseUrl
    Ensure-ProviderName

    # 若 API key 已存在,直接跳過 setup
    if ((Test-ApiKey)) { return }

    # API key 未設定 -> 只問 key
    Write-LInfo '設定 LiteLLM API Key'
    Write-Host ''
    Prompt-ApiKey
    Write-Host ''
